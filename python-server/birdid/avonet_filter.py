"""
AvonetFilter - Offline species filter based on avonet.db.

Uses AVONET global bird distribution data for offline species filtering,
replacing the eBird API which requires network access.

Original code from SuperPicky by Sun Jiao.
"""
...
class AvonetFilter:
    def __init__(self, db_path: Optional[str] = None):
        if db_path is None:
            db_path = os.path.join(
                os.path.dirname(__file__), "data", "avonet.db"
            )
        ...
        if self.db_path and os.path.exists(self.db_path):
            try:
                ...
                logger.warning("Avonet DB connection failed: %s", e)
                self._conn = None
    ...
    def _load_ebird_cls_map(self) -> dict:
        ...
        map_path = os.path.join(
            module_dir, "data", "ebird_classid_mapping.json"
        )
    ...
    def _codes_to_class_ids(self, ebird_codes: List[str]) -> Set[int]:
        cls_map = self._load_ebird_cls_map()
        return {cls_map[code] for code in ebird_codes if code in cls_map}

    def _load_ebird_species_file(
        self, code: str
    ) -> Optional[List[str]]:
        module_dir = os.path.dirname(os.path.abspath(__file__))
        path = os.path.join(
            module_dir, "data", "offline_ebird_data",
            f"species_list_{code}.json"
        )
    ...
    def get_species_by_country_ebird(
        ...
        if not country_code:
            return set(), None
        species_codes = self._load_ebird_species_file(country_code)
        if not species_codes:
            return set(), None
        return self._codes_to_class_ids(species_codes), country_code

    def get_species_by_region_ebird(
        self, region_code: str
    ) -> Tuple[Set[int], Optional[str]]:
        region_code = region_code.upper()
        species_codes = self._load_ebird_species_file(region_code)
        actual_region = region_code

        if not species_codes and "-" in region_code:
            country = region_code.split("-")[0]
            species_codes = self._load_ebird_species_file(country)
            actual_region = country if species_codes else None

        if not species_codes:
            return set(), None
        return self._codes_to_class_ids(species_codes), actual_region

    def get_filter_chain(
        self, lat: Optional[float], lon: Optional[float]
    ) -> List[Optional[Set[int]]]:
        """Return fallback chain: GPS grid -> country eBird -> global(None).

        Each entry is either a Set[int] of class IDs or None (meaning
        no filter / allow all species).
        """
        if lat is None or lon is None:
            return [None]

        chain: List[Optional[Set[int]]] = []

        # Level 1: GPS grid lookup
        gps_species = self.get_species_by_gps(lat, lon)
        if gps_species:
            chain.append(gps_species)

        # Level 2: Country eBird lookup
        country_species, _ = self.get_species_by_country_ebird(lat, lon)
        if country_species:
            chain.append(country_species)

        # Level 3: Global fallback (None = no filter)
        chain.append(None)
        return chain
